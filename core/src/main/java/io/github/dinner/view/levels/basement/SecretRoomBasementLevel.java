...
public class SecretRoomBasementLevel extends LevelState {
    private String floorType = "wood";

    public SecretRoomBasementLevel() {
        super();

        if(Player.getPlayer().getGender() == 'F')
            this.map = new TmxMapLoader().load("map/Basement/SecretRoom/SecretRoomF.tmx");
        if(Player.getPlayer().getGender() == 'M')
            this.map = new TmxMapLoader().load("map/Basement/SecretRoom/SecretRoomM.tmx");
    ...
    @Override
    public void init() {
        InteractableObject corridorBasementEntrance = new InteractableObject(new Rectangle(7, 6, 1, 1), true) {
            @Override
        ...
        interactableObjectController.add(corridorBasementEntrance);
    }
