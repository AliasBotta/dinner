...
public class BallroomFFLevel extends LevelState {
    private String floorType = "wood";

    public BallroomFFLevel() {
        super();

        this.map = new TmxMapLoader().load("map/1st Floor/Ballroom/Ballroom.tmx");
        ...
        this.name = "ballroomFFLevel";
    }

    @Override
    public void init() {
        InteractableObject westCorridorEntrance = new InteractableObject(new Rectangle(20, 8, 1, 2), true) {
            @Override
            public void interact(Player player) {
                TransitionScreen fadeScreen = new TransitionScreen(
                    ...
                    16, Player.getPlayer().getBox().y == 8 ? 5 : 6,
